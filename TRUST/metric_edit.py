...
def inference_graph(loader, model, device, view, data_size, alpha, class_num):
    """
    :return:
    total_pred: prediction among all modalities
    pred_vectors: predictions of each modality, list
    ...
    for step, (xs, y, _) in enumerate(loader):
        for v in range(view):
            xs[v] = xs[v].to(device)
        with torch.no_grad():
            hs, _, _, zs, Cs, _ = model.forward(xs, 0, 0)
        ...
        L = L / L.max()
        L = 0.5 * (L + L.T)
        spectral = cluster.SpectralClustering(n_clusters=class_num, eigen_solver='arpack', affinity='precomputed',
...
def valid(model, device, dataset, view, data_size, class_num, eval_h=False):
    ...
    total_pred, labels_vector = inference_graph(test_loader, model, device, view, data_size, alpha, class_num)
